...
export class StaffService {
  ...
  getStaffMemberByUserId(userId: number): Observable<Staff> {
    return this.apiService.get<Staff>(`${this.endpoint}/${userId}`);
  }

  getStaffByDepartment(departmentId: number): Observable<Staff[]> {
    return this.apiService.get<Staff[]>(
      `${this.endpoint}/departments/${departmentId}`
    );
  }
  ...
  updateStaffByUserId(
    ...
    return this.apiService.patch<Staff>(
